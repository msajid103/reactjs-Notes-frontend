...
const NoteCard = ({ data }) => {
    let body = data.body.split(' ').slice(0, 20).join(' ')
    const color = data.category=='BUSINESS'?'blue':data.category == "PERSONAL"? 'green':'purple';``
    ...
    return (
        <div className="col-md-4 single-note-item all-category">
            <div className="card card-body">
                ...
                <Link to={`/notes/${data.slug}`} style={{ textDecoration: "none", color: 'black' }}>
                    <h5 className="note-title text-truncate w-75 mb-0" data-noteheading="Book a Ticket for Movie">{data.title} </h5>
                </Link>
                <p className="note-date font-12 text-muted">{FormatDate(data.updated)}</p>
                        ...
                        <div className="category-selector btn-group">
                            <a className="nav-link dropdown-toggle category-dropdown label-group p-0" data-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="true">
                                <div className="category">
                                    ...
                                    <span className="more-options text-dark"><i className="icon-options-vertical"></i>{data.category}</span>
                                </div>
                            </a>
